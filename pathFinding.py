...
# 0：非DEBUG,将会执行完当前小地图
# 1：区域内地图点击，及行动DEBUG，不执行区域内战斗，若有怪物，可能会被打断
...
def _action(actionSequence: tuple):
    ...
    for actionI in actionSequence:
        print(actionI)
        if actionI[0] == 'x': #移动横向视角，一圈大概7720，非精准操纵，慎用，负数为向左转，正数为向右转
            pdi.moveRel(xOffset=actionI[1], yOffset=0, relative=True)
        if actionI[0] == 'y': #移动纵向视角，非精准操纵，慎用，甚至感觉没用(ˉ▽ˉ；)...
            #一圈大概7720
            pdi.moveRel(xOffset=0, yOffset=actionI[1], relative=True)
        elif actionI[0] == 'c': #左键，平A
            if DEBUG == 1:
                return 0
            pa.click()
            time.sleep(1)
        elif actionI[0] == 'CF': #检测战斗是否结束
            if DEBUG == 1:
                return 0
            aIL = len(actionI)
            if aIL == 1:
        ...
        elif actionI[0] == 'f': #按F，当前版本，仅有进入画卷这一操作需要
            time.sleep(1)
            pa.press(actionI[0])
    ...
    if rN == 0: # 流云渡
        ...
        for rNN in range (0,3):
            ...
            if rNN == 0:
                ...
                aSequence = (['d', 0.3],['w',3],['c'],['a',0.3],['w',2],['wd',5.1],['f',7],
...
if __name__ == '__main__':
    ...
    if 0 == getStarTrain():
        ...
        exit()
    #0开始
    selectRegion(0, 0) # Debug哪个区域直接在这里修改
